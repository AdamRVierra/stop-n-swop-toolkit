...
HEADER_COPY := $(BUILD_DIR)/ipl3_header.z64
$(HEADER_COPY): $(N64_ROM_HEADER) | $(BUILD_DIR)
	@cmp -s "$<" "$@" || cp "$<" "$@"

$(N64_ROM_HEADER): 

stopnswop.z64: $(BUILD_DIR)/stopnswop.elf $(BUILD_DIR)/stopnswop.dfs $(HEADER_COPY)
	...
	$(N64_INST)/bin/n64tool -o "stopnswop.z64" -h "$(HEADER_COPY)" -t "$(ROM_TITLE)" -T --align 256 "$(BUILD_DIR)/stopnswop.elf.stripped" --align 8 "$(BUILD_DIR)/stopnswop.elf.sym" --align 16 "$(BUILD_DIR)/stopnswop.dfs"
